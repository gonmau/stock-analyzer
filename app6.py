...
def load_ticker_map() -> dict:
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            market = str(row.get('시장', '')).strip().upper()
...
ALIAS_MAP = {
    ...
    'NHN엔터테인먼트': 'NHN',
    '포스코아이씨티': '포스코DX',
    # 보통주 접미어 제거 (normalize_stock_name의 regex로도 처리되지만 명시적으로)
    '삼성전자보통주': '삼성전자',
    'LG전자보통주': 'LG전자',
    'SK보통주': 'SK',
    'SKC보통주': 'SKC',
    ...
    '에이프로젠H': '에이프로젠HG',
    '에이프로젠 H&G': '에이프로젠HG',
    # 표시명 통일 (종목키가 붙은 형태로 저장되는 것들)
    'JYPEnt': 'JYP Ent.',
    '주식회사제노포커스': '제노포커스',
    '메타플랫폼스페이스북': '메타플랫폼스',
    '미래에셋TIGER인도니프티50증권상장지수투자신탁주식': '미래에셋TIGER인도니프티50',
    '신한SOL조선TOP3플러스증권상장지수투자신탁주식': '신한SOL조선TOP3플러스',
}
...
def _is_mirae_detail_format(df_raw):
    """
    미래에셋 '자세히 보기' 포맷 여부 감지.
    - 4행(index=3)에 '자세히' 텍스트 포함
    - 5행(index=4) 헤더: 거래일자/거래종류
    - 6행(index=5) 서브헤더: 단가
    """
    try:
...
def _parse_mirae_detail(df_raw):
    """
    미래에셋 자세히 포맷 2행-쌍 파서.
    ...
    while i < len(data):
        ...
        거래수량 = _to_float(row1.iloc[2])
        거래금액 = _to_float(row1.iloc[3])
        수수료   = _to_float(row1.iloc[5])
        제세금   = _to_float(row2.iloc[5])
...
def _calc_positions_single(df):
    ...
    for _, row in df.iterrows():
        ...
        _su, _tax = row.get('수수료', 0), row.get('제세금', 0)
        fee    = (float(_su) if pd.notna(_su) else 0.0) + (float(_tax) if pd.notna(_tax) else 0.0)
